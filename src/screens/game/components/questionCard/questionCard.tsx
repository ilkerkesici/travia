...
import { configureAnswers, determineScoreOfTheQuestion } from 'screens/game/game.helper';
...
const QuestionCard = (props: IQuestionCard) => {
    ...
    const onPressAnswer = useCallback((selectedAnswer: string) => {
        ...
        if (questionInfo.correct_answer === selectedAnswer) { // The answer is correnct
            const score = determineScoreOfTheQuestion(questionInfo.difficulty);
            onCorrect(score);
        } else
            onWrong();
